# synchrofit
The ```synchrofit``` (**synchro**tron **fit**ter) package implements a reduced dimensionality parameterisation (Turner et al. 2018b, Turner 2018) of standard synchrotron spectrum models, and provides fitting routines applicable for active galactic nuclei and supernova remnants. The Python code includes the Jaffe-Parola model (JP; Jaffe & Parola 1973), Kardashev-Pacholczyk model (KP; Kardashev 1962, Pacholczyk 1970), and continuous injection models (CI/KGJP; Kommissarov & Gubanov 1994) for both constant or Maxwell-Boltzmann magnetic field distributions. An adaptive maximum likelihood algorithm is invoked to fit these models to multi-frequency radio-frequency observations; the adaptive mesh is customisable for either optimal precision or computational efficiency. Functions are additionally provided to plot the fitted spectral model with its confidence interval, and to derive the spectral age of the synchrotron emitting particles. 
