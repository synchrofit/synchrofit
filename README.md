...
In this example, `${data.dat}` is a `.dat` file containing the input spectrum (see [test_spectra.dat](https://github.com/synchrofit/Examples) for an example of the format required), and `${fit_type}` describes the model to be fit (e.g. KP, JP, CI, TKP, TJP, TCI). 
